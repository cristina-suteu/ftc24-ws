...
npts = navg * nfft
fs = 750000.0  # Sampling frequency in Hz for m2k DAC

phase = 0.0  # Tone phase
td = 0.0
tj = 0.0
qres = 16  # Quantizer for m2k ADC
...
pl.figure(2)
pl.plot(data_in[:1000])
qnoise = 10 ** (qnoise_dbfs / 20)


# Quantize waveform
#qwf = gn.quantize(np.array(data_in), fsr, qres, qnoise, code_fmt)
